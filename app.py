...
class BingAutomatorApp(customtkinter.CTk):
    ...
    def _create_left_frame(self):
        self.profile_frame = customtkinter.CTkFrame(self.top_frame); self.profile_frame.grid(row=0, column=0, padx=(10, 5), pady=10, sticky="nsew")
        self.profile_frame.grid_rowconfigure(1, weight=1); self.profile_frame.grid_columnconfigure(0, weight=1)
        left_header = customtkinter.CTkFrame(self.profile_frame); left_header.grid(row=0, column=0, sticky="ew"); left_header.grid_columnconfigure(0, weight=1)
        left_title = customtkinter.CTkLabel(left_header, text="Profiles", font=customtkinter.CTkFont(weight="bold")); left_title.grid(row=0, column=0, padx=10, pady=5, sticky="w")
        collapse_left_button = customtkinter.CTkButton(left_header, text="◄", width=25, command=lambda: self._toggle_left_frame()); collapse_left_button.grid(row=0, column=1, padx=5, pady=5, sticky="e")
        profile_content_frame = customtkinter.CTkFrame(self.profile_frame, fg_color="transparent"); profile_content_frame.grid(row=1, column=0, sticky="nsew")
        profile_content_frame.grid_rowconfigure(1, weight=1); profile_content_frame.grid_columnconfigure(0, weight=1)
        search_frame = customtkinter.CTkFrame(profile_content_frame, corner_radius=8, border_width=1); search_frame.grid(row=0, column=0, sticky="ew", padx=5, pady=5); search_frame.grid_columnconfigure(1, weight=1)
        search_icon_label = customtkinter.CTkLabel(search_frame, text="🔍", width=20, font=("Segoe UI Emoji", 16)); search_icon_label.grid(row=0, column=0, padx=(10, 5), pady=5)
        self.search_var = customtkinter.StringVar(); self.search_entry = customtkinter.CTkEntry(search_frame, placeholder_text="Search profiles...", textvariable=self.search_var, border_width=0, fg_color="transparent")
        self.search_entry.grid(row=0, column=1, sticky="ew", pady=5, padx=(0, 5)); self.search_var.trace_add("write", self._filter_profiles)
        self.scrollable_frame = customtkinter.CTkScrollableFrame(profile_content_frame); self.scrollable_frame.grid(row=1, column=0, sticky="nsew", padx=5, pady=5)
        for profile in self.profiles:
            ...
            profile_widget.pack(fill="x", expand=True, padx=5, pady=2); self.profile_widget_map[profile] = profile_widget

    def _create_left_frame_collapsed(self):
        self.left_frame_collapsed = customtkinter.CTkFrame(self.top_frame); self.left_frame_collapsed.grid_columnconfigure(0, weight=1)
        left_title = customtkinter.CTkLabel(self.left_frame_collapsed, text="Profiles", font=customtkinter.CTkFont(weight="bold")); left_title.grid(row=0, column=0, padx=10, pady=5, sticky="w")
        expand_left_button = customtkinter.CTkButton(self.left_frame_collapsed, text="►", width=25, command=lambda: self._toggle_left_frame()); expand_left_button.grid(row=0, column=1, padx=5, pady=5, sticky="e")

    def _create_right_frame(self):
        self.controls_frame = customtkinter.CTkFrame(self.top_frame); self.controls_frame.grid(row=0, column=1, padx=(5, 10), pady=10, sticky="nsew")
        self.controls_frame.grid_rowconfigure(1, weight=1); self.controls_frame.grid_columnconfigure(0, weight=1)
        right_header = customtkinter.CTkFrame(self.controls_frame); right_header.grid(row=0, column=0, sticky="ew"); right_header.grid_columnconfigure(0, weight=1)
        right_title = customtkinter.CTkLabel(right_header, text="Controls", font=customtkinter.CTkFont(weight="bold")); right_title.grid(row=0, column=0, padx=10, pady=5, sticky="w")
        ...
        self.post_search_delay_slider = self.RangeSlider(behavior_frame, text="Post-Search Pause (s):", min_val=0.5, max_val=15.0, step=0.1, initial_min=self.settings["post_search_delay_min"], initial_max=self.settings["post_search_delay_max"], command=self._save_settings); self.post_search_delay_slider.pack(fill="x", padx=10, pady=5)
        self.scroll_delay_slider = self.RangeSlider(behavior_frame, text="Scroll Action Pause (s):", min_val=0.1, max_val=5.0, step=0.1, initial_min=self.settings["scroll_delay_min"], initial_max=self.settings["scroll_delay_max"], command=self._save_settings); self.scroll_delay_slider.pack(fill="x", padx=10, pady=5)
        self.mouse_move_slider = self.RangeSlider(behavior_frame, text="Mouse Move Duration (s):", min_val=0.1, max_val=1.0, step=0.05, initial_min=self.settings["mouse_move_duration_min"], initial_max=self.settings["mouse_move_duration_max"], command=self._save_settings); self.mouse_move_slider.pack(fill="x", padx=10, pady=5)
        self.key_press_slider = self.RangeSlider(behavior_frame, text="Key Press Delay (s):", min_val=0.01, max_val=0.5, step=0.01, initial_min=self.settings["key_press_delay_min"], initial_max=self.settings["key_press_delay_max"], command=self._save_settings, value_format="{:.3f}"); self.key_press_slider.pack(fill="x", padx=10, pady=5)
        scheduler_frame = customtkinter.CTkFrame(self.controls_content_frame); scheduler_frame.pack(fill="x", padx=10, pady=10); scheduler_frame.grid_columnconfigure(1, weight=1)
        scheduler_title = customtkinter.CTkLabel(scheduler_frame, text="Daily Scheduler", font=customtkinter.CTkFont(weight="bold")); scheduler_title.grid(row=0, column=0, columnspan=2, padx=10, pady=(5, 10), sticky="w")
        self.schedule_switch_var = customtkinter.StringVar(value="on" if self.settings["schedule_enabled"] else "off"); schedule_switch = customtkinter.CTkSwitch(scheduler_frame, text="Enable Schedule", variable=self.schedule_switch_var, onvalue="on", offvalue="off", command=self._toggle_schedule); schedule_switch.grid(row=1, column=0, columnspan=2, padx=10, pady=5, sticky="w")
        schedule_time_label = customtkinter.CTkLabel(scheduler_frame, text="Run at (24h format):"); schedule_time_label.grid(row=2, column=0, padx=10, pady=(0, 10))
        self.schedule_time_entry = customtkinter.CTkEntry(scheduler_frame, width=60); self.schedule_time_entry.insert(0, self.settings["schedule_time"]); self.schedule_time_entry.grid(row=2, column=1, padx=10, pady=(0, 10), sticky="w")
        self.schedule_time_entry.bind("<FocusOut>", self._save_schedule_time); self.schedule_time_entry.bind("<Return>", self._save_schedule_time)
        theme_label = customtkinter.CTkLabel(self.controls_content_frame, text="Theme:"); theme_label.pack(padx=10, pady=(10, 0), anchor="w")
        self.theme_switcher = customtkinter.CTkSegmentedButton(self.controls_content_frame, values=["Light", "Dark", "System"], command=self._theme_switch_callback); self.theme_switcher.set("System"); self.theme_switcher.pack(fill="x", padx=10, pady=(5, 10))

    def _create_right_frame_collapsed(self):
        self.right_frame_collapsed = customtkinter.CTkFrame(self.top_frame); self.right_frame_collapsed.grid_columnconfigure(0, weight=1)
        right_title = customtkinter.CTkLabel(self.right_frame_collapsed, text="Controls", font=customtkinter.CTkFont(weight="bold")); right_title.grid(row=0, column=0, padx=10, pady=5, sticky="w")
        expand_right_button = customtkinter.CTkButton(self.right_frame_collapsed, text="◄", width=25, command=lambda: self._toggle_right_frame()); expand_right_button.grid(row=0, column=1, padx=5, pady=5, sticky="e")
    ...
    def _scroll_to_profile(self, profile: EdgeProfile):
        ...
        if widget_to_scroll_to:
            self.update_idletasks()
            try:
                canvas_height = self.scrollable_frame._parent_canvas.winfo_height(); widget_height = widget_to_scroll_to.winfo_height(); widget_y = widget_to_scroll_to.winfo_y()
                scroll_position = widget_y - (canvas_height / 2) + (widget_height / 2); scrollable_content_height = self.scrollable_frame._parent_canvas.bbox("all")[3]
                if scrollable_content_height > 0: final_position = scroll_position / scrollable_content_height; clamped_position = max(0.0, min(1.0, final_position)); self.scrollable_frame._parent_canvas.yview_moveto(clamped_position)
            except Exception as e: logger.log(f"Error scrolling to profile: {e}", "WARN")

    def _update_status_counts(self):
        active_count = sum(1 for p in self.profiles if p.status == 'active'); suspended_count = len(self.profiles) - active_count
        self.active_count_label.configure(text=f"Active: {active_count}", text_color="lightgreen"); self.suspended_count_label.configure(text=f"Suspended: {suspended_count}", text_color="orange")

    def _on_status_toggle(self, profile: EdgeProfile): self._update_status(f"Profile {profile.name} set to '{profile.status}'."); self._save_all_profiles_to_json(); self._update_status_counts()
    def _save_all_profiles_to_json(self):
        all_profiles_data = {};
        for p in self.profiles: all_profiles_data[p.full_name] = p.to_dict()
        try:
            with open(config.PROFILES_JSON_PATH, 'w', encoding='utf-8') as f: json.dump(all_profiles_data, f, indent=2); # Removed excessive logging
        except Exception as e: logger.log(f"Error saving profile data: {e}", "ERROR")

    def _load_and_display_initial_progress(self):
        todays_progress = self.automation_service.load_todays_progress_from_history(); profile_email_map = {p.email: p for p in self.profiles}
        for email, progress_data in todays_progress.items():
            profile = profile_email_map.get(email);
            if profile: widget = self.profile_widget_map.get(profile);
            if widget: widget.update_points_display(progress_data)
    ...
    def _start_automation_thread(self):
        self.stop_event = threading.Event(); profiles_to_run = sorted([p for p in self.selected_profiles if p.status == 'active'], key=lambda p: p.index)
        if not profiles_to_run: self._update_status("No active profiles selected. Nothing to do."); return
    ...
    def _start_fetch_progress_thread(self):
        self.stop_event = threading.Event(); profiles_to_run = sorted(list(self.selected_profiles), key=lambda p: p.index)
        if not profiles_to_run: self._update_status("No profiles selected. Nothing to do."); return
        self.start_button.configure(state="disabled"); self.fetch_progress_button.configure(text="Stop", command=self._stop_automation, fg_color="red", hover_color="#C40000")
        thread = threading.Thread(target=self._fetch_progress_worker, args=(profiles_to_run, self.stop_event), daemon=True); thread.start()
    ...
    def _fetch_progress_worker(self, profiles_to_run: List[EdgeProfile], stop_event: threading.Event):
        try:
            ...
            if not active_profiles_to_run: self._update_status("No active profiles selected to fetch."); return
            total_profiles = len(active_profiles_to_run); self.selenium_lock.acquire()
    ...
    def _run_scheduled_tasks(self):
        ...
        stop_event = threading.Event(); profiles_to_run = [p for p in self.profiles if p.status == 'active']
    ...
    def _update_all_checkbox_state(self):
        if len(self.selected_profiles) == len(self.profiles): self.all_check_var.set("on")
        else: self.all_check_var.set("off")
        self._update_all_checkbox_text()

    def _update_all_checkbox_text(self): self.all_checkbox.configure(text=f"All ({len(self.selected_profiles)}/{len(self.profiles)})")
    def _update_selection_ui(self):
        for widget in self.profile_widget_map.values(): widget.set_checked(widget.profile in self.selected_profiles)
        self._update_all_checkbox_state()

    def _update_option_menu(self, value=None):
        batch_size = self.batch_slider.get(); num_profiles = len(self.profiles)
        options = [f"{i + 1}-{min(i + batch_size, num_profiles)}" for i in range(0, num_profiles, batch_size)]
        full_options = ["Options", "Inverse Selection", "Selected Info", "Custom Range...", "Auto-detect Profiles", "Open Log File", "Clear Log File", "View History", "Clear History File"] + options
    ...
    def _optionmenu_callback(self, choice: str):
        if choice == "Inverse Selection":
            ...
            if self.automation_service.clear_history_file():
                ...
                for widget in self.profile_widget_map.values():
                    widget.update_points_display({}) # Reset UI display
            else:
                self._update_status("Could not clear history file. Check logs.")
        elif '-' in choice: # Handle range selection like "1-4"
            try:
                start, end = map(int, choice.split('-'))
                # Adjust for 0-based index vs 1-based display
                self.selected_profiles = set(self.profiles[start - 1 : end])
                self._update_selection_ui()
        ...
        # Reset dropdown text after selection
    ...
    def _theme_switch_callback(self, value: str): logger.log(f"Theme changed to {value}", level="SYSTEM"); customtkinter.set_appearance_mode(value.lower())
